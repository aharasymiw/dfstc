app.controller('FormCtrl', ['$scope', '$http', 'store',
function($scope, $http, store) {
  ...
  var updateAppointments = function() {
    $http({
      ...
      data: {_id: $scope.form.appointment, email: $scope.form.clientEmail,
      appointmentType: $scope.form.appointmentType}
    }).then(function successCallback(response) {
      saveClient();
      console.log(response);
    }, function errorCallback(response) {
  ...
  $scope.states = ('AL AK AZ AR CA CO CT DE FL GA HI ID IL IN IA KS KY LA ME ' +
   ...
   'UT VT VA WA WV WI WY').split(' ').map(function(state) {
    return {abbrev: state};
  });
}])
